...
[![NPM Package](https://img.shields.io/npm/v/happyucjs-wallet.svg?style=flat-square)](https://www.npmjs.org/package/happyucjs-wallet)
[![Build Status](https://travis-ci.org/happyucjs/happyucjs-wallet.svg?branch=master)](https://travis-ci.org/happyucjs/happyucjs-wallet)
[![Coverage Status](https://img.shields.io/coveralls/happyucjs/happyucjs-wallet.svg?style=flat-square)](https://coveralls.io/r/happyucjs/happyucjs-wallet)
...
- [happyucjs-tx](https://github.com/happyucjs/happyucjs-tx) to sign transactions
- [happyucjs-icap](https://github.com/happyucjs/happyucjs-icap) to manipulate ICAP addresses
- [store.js](https://github.com/marcuswestin/store.js) to use browser storage

Motivations are:
- be lightweight
- work in a browser
- use a single, maintained version of crypto library (and that should be in line with `happyucjs-util` and `happyucjs-tx`)
...
* `fromV1(input, password)` - import a wallet (Version 1 of the HappyUC wallet format)
* `fromV3(input, password, [nonStrict])` - import a wallet (Version 3 of the HappyUC wallet format). Set `nonStrict` true to accept files with mixed-caps.
* `fromHucSale(input, password)` - import an HappyUC Pre Sale wallet

For the V1, V3 and HucSale formats the input is a JSON serialized string. All these formats require a password.

Note: `fromPublicKey()` only accepts uncompressed HappyUC-style public keys, unless the `nonStrict` flag is set to true.
...
* `getChecksumAddressString()` - return the [address with checksum](https://github.com/happyuc-project/EIPs/issues/55)
* `getV3Filename([timestamp])` - return the suggested filename for V3 keystores
* `toV3(password, [options])` - return the wallet as a JSON string (Version 3 of the HappyUC wallet format)
...
Note: `getPublicKey()` only returns uncompressed HappyUC-style public keys.

## Thirdparty API

Importing various third party wallets is possible through the `thirdparty` submodule:

`var thirdparty = require('happyucjs-wallet/thirdparty')`

Constructors:

* `fromHucCamp(passphrase)` - import a brain wallet used by Huc.Camp
* `fromHucWallet(input, password)` - import a wallet generated by HucWallet
...
`var hdkey = require('happyucjs-wallet/hdkey')`
...
```js
const WalletSubprovider = require('happyucjs-wallet/provider-engine')
...
The following settings are favoured by the Go HappyUC implementation and we default to the same:
- `kdf`: `scrypt`
- `dklen`: `32`
- `n`: `262144`
- `r`: `8`
